...
def load_options(fname):
    ...
    lines = f.readlines()
    f.close()
    # options that represent files or folders
    files_opts = ["INPUT_LOC","OUTPUT_LOC","DATA_LOC","ATTACK_LOC"]
    cwd = os.getcwd()+"/"
    for line in lines:
        ignore = 0
        if (len(line) > 0):
        ...
        if (ignore == 0):
            ...
            li = line.split()
            val = parsed(li[1])
            if li[0] in files_opts:
                if "LOC" in li[0] and val[-1] != "/":
                    val = val+'/'
                if val[0] != "/":
                    # this is not an absolute path, add path of current directory
                    val = cwd+val
            d_options[li[0]] = val
    return d_options
