...
def calculate_bw_threshold(files, timestamp_column=0, packet_size_column=1):
...
def extract_times(files, output="", neighbors=2, timestamp_column=0, packet_size_column=1):
    '''
    ...
    print('Computing bandwidth thresholds with %d files...' % len(files))
    outgoing_bw_thr,incoming_bw_thr,traces = calculate_bw_threshold(files, timestamp_column, packet_size_column)  # returns upload and download bw reps #
    ...
    for trace_ in traces:
        ...
        for direction in [INCOMING, OUTGOING]:
            ...
            for ind, pkt in enumerate(trace):
                # trace here is purely incoming or outgoing
                current_bw = get_current_bw(trace, ind, neighbors, direction)
                if current_bw == -1:
                    current_bw = prev_bw # prev_bw should be updated

                if current_bw > bw_threshold and start_burst == 1:
                    # Here we want to calculate and collect all iats within bursts
                    if ind-1 >= 0:
                        iat = trace[ind][0] - trace[ind-1][0]
                        if direction == INCOMING:
                            server_GAP_send_histogram.append(iat)
    ...
    if output != "":
        ...
        print("Proportion of 0 in client_GAP_send_histogram is %f"
              % (float(len([x for x in client_GAP_send_histogram if x == 0]))/len(client_GAP_send_histogram)))
        print("Proportion of 0 in server_GAP_send_histogram is %f"
              % (float(len([x for x in server_GAP_send_histogram if x == 0]))/len(server_GAP_send_histogram)))
        ...
        print("Proportion of 0 in client_BURST_send_histogram is %f"
              % (float(len([x for x in client_BURST_send_histogram if x == 0]))/len(client_BURST_send_histogram)))
        print("Proportion of 0 in server_BURST_send_histogram is %f"
              % (float(len([x for x in server_BURST_send_histogram if x == 0]))/len(server_BURST_send_histogram)))
        ...
        print("Proportion of 0 in client_BURST_receive_histogram is %f"
              % (float(len([x for x in client_BURST_receive_histogram if x == 0]))/len(client_BURST_receive_histogram)))
        print("Proportion of 0 in server_BURST_receive_histogram is %f"
              % (float(len([x for x in server_BURST_receive_histogram if x == 0]))/len(server_BURST_receive_histogram)))
        ...
        print("\nThe median of burst length in outgoing traffic: %f" % np.median(np.array(outgoing_burst_length)))
        print("\nThe median of burst length in incoming traffic: %f" % np.median(np.array(incoming_burst_length)))
        print("\nDone! Saving files...\n")

    return client_GAP_send_histogram,client_BURST_send_histogram,client_BURST_receive_histogram, \
           server_GAP_send_histogram, server_BURST_send_histogram, server_BURST_receive_histogram, \
           outgoing_burst_length,incoming_burst_length, number_of_burst_outgoing, number_of_burst_incoming
...
def parse_arguments():
    ...
    parser.add_argument('--arg', '-a',
                        ...
                        type=str,
                        metavar='<arg>',
                        help='Arg that must be present in file names')
...
def main():
    ...
    all_files = []
    for root, dirs, files in os.walk(args.traces_path):
        for f in files:
            if args.arg not in f.split("/")[-1]:
                continue
    ...
    number_of_burst_outgoing, number_of_burst_incoming = extract_times(all_files, args.output, args.neighbors,
                                                                       args.timestamp_column, args.packet_size_column)
